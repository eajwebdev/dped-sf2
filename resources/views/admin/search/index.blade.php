        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <x-card title="Students ({{ $results['students']->count() }})">
                @forelse ($results['students'] as $s)
                    <a href="{{ route('admin.students.show', $s) }}" class="flex items-center justify-between rounded-lg px-2 py-1.5 hover:bg-gray-50 dark:hover:bg-navy-700/40">
            ...
            <x-card title="Teachers ({{ $results['teachers']->count() }})">
                @forelse ($results['teachers'] as $t)
                    <a href="{{ route('admin.teachers.edit', $t) }}" class="block rounded-lg px-2 py-1.5 text-sm hover:bg-gray-50 dark:hover:bg-navy-700/40">{{ $t->full_name }}</a>
            ...
            <x-card title="Sections ({{ $results['sections']->count() }})">
                @forelse ($results['sections'] as $sec)
                    <a href="{{ route('admin.assignments.index', $sec) }}" class="block rounded-lg px-2 py-1.5 text-sm hover:bg-gray-50 dark:hover:bg-navy-700/40">{{ $sec->gradeLevel->name }} — {{ $sec->name }} <span class="text-xs text-gray-400">({{ $sec->schoolYear->name }})</span></a>
            ...
            <x-card title="Subjects ({{ $results['subjects']->count() }})">
                @forelse ($results['subjects'] as $sub)
                    <a href="{{ route('admin.subjects.edit', $sub) }}" class="block rounded-lg px-2 py-1.5 text-sm hover:bg-gray-50 dark:hover:bg-navy-700/40">{{ $sub->name }} <span class="text-xs text-gray-400">({{ $sub->code }})</span></a>
