    ...
    <div x-data="{
            ...
            <form method="GET" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div class="relative">
                    <input type="search" name="q" value="{{ $search }}" placeholder="Search LRN or name…"
                           class="w-full sm:w-64 rounded-lg border border-gray-300 dark:border-white/15 dark:bg-navy-900 text-sm pl-4 pr-10 py-2.5 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all">
                ...
                <select name="status" onchange="this.form.submit()" class="rounded-lg border border-gray-300 dark:border-white/15 dark:bg-navy-900 text-sm px-4 py-2.5 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all">
                    <option value="">All statuses</option>
                    @foreach (['active','transferred','dropped','graduated','inactive'] as $st)
                        <option value="{{ $st }}" @selected(request('status') === $st)>{{ ucfirst($st) }}</option>
                    @endforeach
                </select>
                <button type="submit" class="rounded-lg bg-gray-200 dark:bg-navy-700 px-6 py-2.5 text-sm font-medium hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">Search</button>
            </form>
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                <a href="{{ route('admin.students.export', request()->only('q','status')) }}" class="inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 dark:border-white/15 px-4 py-2.5 text-sm font-medium hover:bg-gray-50 dark:hover:bg-navy-700/50 transition-colors" title="Export students">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                    Export
                </a>
                <button @click="importOpen = true" class="inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 dark:border-white/15 px-4 py-2.5 text-sm font-medium hover:bg-gray-50 dark:hover:bg-navy-700/50 transition-colors" title="Import students">
                ...
                <button @click="createOpen = true" class="inline-flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-brand-600 to-brand-700 px-6 py-2.5 text-sm font-bold text-white hover:shadow-lg hover:shadow-brand-500/30 transition-all">
        ...
        <div x-show="createOpen" x-cloak x-transition:enter="transition ease-out duration-200"
            ...
            <div x-show="createOpen" x-transition:enter="transition ease-out duration-200"
                 ...
                 class="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white dark:bg-navy-800 shadow-2xl">
                <div class="sticky top-0 flex items-center justify-between border-b border-gray-200 dark:border-white/10 px-6 py-5 bg-gradient-to-r from-brand-50 to-blue-50 dark:from-navy-800 dark:to-gray-700">
                    <div>
                    ...
                    <button @click="createOpen = false" class="flex-shrink-0 p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors hover:bg-gray-100 dark:hover:bg-navy-700 rounded-lg">
        ...
        <div x-show="editOpen" x-cloak x-transition:enter="transition ease-out duration-200"
            ...
            <div x-show="editOpen" x-transition:enter="transition ease-out duration-200"
                 ...
                 x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95"
                 class="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white dark:bg-navy-800 shadow-2xl">
                <div class="sticky top-0 flex items-center justify-between border-b border-gray-200 dark:border-white/10 px-6 py-5 bg-gradient-to-r from-blue-50 to-brand-50 dark:from-navy-800 dark:to-gray-700">
                    <div>
                    ...
                    <button @click="editOpen = false" class="flex-shrink-0 p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors hover:bg-gray-100 dark:hover:bg-navy-700 rounded-lg">
                        <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                </div>
                <div class="p-6 overflow-y-auto">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <form method="POST" action="{{ route('admin.students.import') }}" enctype="multipart/form-data" class="space-y-4">
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-white/10">
                            <a href="{{ route('admin.students.import.template') }}" class="inline-flex items-center gap-1 text-xs font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300 transition-colors">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                Download Template
                            </a>
                            <div class="flex gap-3">
                                ...
                                <button type="submit" class="rounded-lg bg-gradient-to-r from-brand-600 to-brand-700 px-5 py-2.5 text-sm font-bold text-white hover:shadow-lg hover:shadow-brand-500/30 transition-all">Import</button>
                            </div>
                        </div>
                    </form>
                </div>
        ...
        {{-- Students Table --}}
        <div class="rounded-2xl bg-white dark:bg-navy-800 shadow-sm border border-gray-200 dark:border-white/10 overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 dark:bg-navy-800/50 border-b border-gray-200 dark:border-white/10">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-navy-700/30 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @else
                                            <span class="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-brand-500 to-brand-700 text-sm font-bold text-white">{{ strtoupper(substr($s->first_name,0,1).substr($s->last_name,0,1)) }}</span>
                                        @endif
                                        <div>
                                            <a href="{{ route('admin.students.show', $s) }}" class="font-semibold text-gray-900 dark:text-white hover:text-brand-600 dark:hover:text-brand-400 transition-colors">{{ $s->full_name }}</a>
                                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $s->gender }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <code class="text-xs font-mono bg-gray-100 dark:bg-navy-900 px-2.5 py-1.5 rounded-lg text-gray-700 dark:text-gray-300">{{ $s->lrn }}</code>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    @php
                                        $badge = [
                                            ...
                                            'graduated' => ['bg' => 'bg-brand-100 dark:bg-brand-500/15', 'text' => 'text-brand-700 dark:text-brand-300', 'dot' => 'bg-brand-500'],
                                            'inactive' => ['bg' => 'bg-gray-100 dark:bg-navy-700', 'text' => 'text-gray-700 dark:text-gray-300', 'dot' => 'bg-gray-500'],
                                        ][$s->status] ?? ['bg' => 'bg-gray-100 dark:bg-navy-700', 'text' => 'text-gray-700 dark:text-gray-300', 'dot' => 'bg-gray-500'];
                                    @endphp
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.students.show', $s) }}" class="inline-flex items-center justify-center p-2.5 rounded-lg text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-navy-700 hover:text-gray-900 dark:hover:text-white transition-colors" title="View Student">
                                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                        </a>
                                        <button @click="editStudent = @json($s); editOpen = true" class="inline-flex items-center justify-center p-2.5 rounded-lg text-brand-600 dark:text-brand-400 hover:bg-brand-50 dark:hover:bg-brand-500/10 hover:text-brand-700 dark:hover:text-brand-300 transition-colors" title="Edit Student">
