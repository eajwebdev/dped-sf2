<x-admin-layout :title="$student->full_name">
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2 space-y-4">
            <x-card>
                <x-slot name="actions">
                    <x-action icon="qr" :href="route('qr-cards.student', $student)" title="Download QR ID card" color="gray" />
                ...
                <div class="flex items-start gap-5">
                    ...
                    @else
                        <span class="flex h-24 w-24 items-center justify-center rounded-xl bg-gray-200 dark:bg-gray-700 text-2xl font-semibold">{{ strtoupper(substr($student->first_name,0,1).substr($student->last_name,0,1)) }}</span>
                    @endif
                    <div class="min-w-0">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
                            @forelse ($student->enrollments as $e)
                                <tr>
                                    ...
                                    <td class="px-3 py-2"><span class="rounded-full bg-gray-100 px-2 py-0.5 text-[11px] capitalize dark:bg-gray-700">{{ str_replace('_',' ',$e->status) }}</span></td>
