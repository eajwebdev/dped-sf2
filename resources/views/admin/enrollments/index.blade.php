<x-admin-layout title="Enrollment">
    ...
    <form method="GET" class="mb-4 flex flex-wrap items-end gap-3">
        <div>
            ...
            <select name="school_year_id" onchange="this.form.submit()" class="mt-1 rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 text-sm focus:border-indigo-500 focus:ring-indigo-500">
        ...
        <div>
            ...
            <select name="section_id" onchange="this.form.submit()" class="mt-1 rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 text-sm focus:border-indigo-500 focus:ring-indigo-500">
        ...
        <div class="grid grid-cols-1 gap-4 xl:grid-cols-2">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
                            @forelse ($roster as $e)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                    <td class="px-2 py-2">
                                        <a href="{{ route('admin.students.show', $e->student) }}" class="font-medium hover:text-indigo-600">{{ $e->student->full_name }}</a>
                                        <span class="block font-mono text-[11px] text-gray-400">{{ $e->student->lrn }}</span>
                                    </td>
                                    <td class="px-2 py-2"><span class="rounded-full bg-gray-100 px-2 py-0.5 text-[11px] capitalize dark:bg-gray-700">{{ str_replace('_',' ',$e->status) }}</span></td>
                                    <td class="px-2 py-2">
                                        <div class="flex items-center justify-end gap-1">
                                            {{-- Transfer within the same year --}}
                                            <form method="POST" action="{{ route('admin.enrollments.transfer', $e) }}">
                                                @csrf @method('PATCH')
                                                <select name="section_id" onchange="this.form.submit()" class="rounded-md border-gray-200 dark:border-gray-600 dark:bg-gray-900 text-[11px] py-1 focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <x-card title="Add Learners">
                <form method="GET" class="mb-3 flex gap-2">
                    ...
                    <input type="search" name="q" value="{{ $search }}" placeholder="Search unassigned learners…" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                    <button class="rounded-lg bg-gray-100 dark:bg-gray-700 px-3 text-sm">Find</button>
                </form>
                ...
                <form method="POST" action="{{ route('admin.enrollments.store') }}" x-data="{ any: false }">
                    ...
                    <div class="max-h-80 overflow-y-auto rounded-lg border border-gray-200 dark:border-gray-700">
                        @forelse ($available as $stud)
                            <label class="flex items-center gap-3 border-b border-gray-100 dark:border-gray-700/50 px-3 py-2 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/30 cursor-pointer">
                                <input type="checkbox" name="student_ids[]" value="{{ $stud->id }}" @change="any = $el.closest('form').querySelectorAll('input[name=\'student_ids[]\']:checked').length > 0"
                                       class="rounded border-gray-300 dark:border-gray-600 dark:bg-gray-900 text-indigo-600 focus:ring-indigo-500">
                    ...
                    <div class="mt-3 flex items-center justify-between">
                        <label class="flex items-center gap-2 text-xs text-gray-500">
                            <input type="checkbox" name="is_late_enrollment" value="1" class="rounded border-gray-300 dark:border-gray-600 dark:bg-gray-900 text-indigo-600 focus:ring-indigo-500">
                            Mark as late enrollment (beyond June cut-off)
                        </label>
                        <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50" :disabled="!any">Enroll selected</button>
                    </div>
                </form>
            </x-card>
        </div>
