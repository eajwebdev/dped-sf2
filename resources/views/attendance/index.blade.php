<x-app-shell title="Attendance">
    <div class="overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
        <div class="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 px-6 py-4">
            ...
            <a href="{{ route('portal') }}" class="inline-flex items-center gap-1.5 text-sm font-medium text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors">
            ...
            @forelse ($sections as $section)
                <div class="flex flex-col gap-3 px-6 py-4 transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/30 sm:flex-row sm:items-center sm:justify-between">
                    <a href="{{ route('attendance.sheet', ['section' => $section, 'date' => $date->toDateString()]) }}" class="group min-w-0 flex-1">
                        <p class="font-semibold text-gray-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{{ $section->gradeLevel->name }} — {{ $section->name }}</p>
                    ...
                    <div class="flex items-center gap-2 self-end sm:self-auto">
                        <a href="{{ route('qr-cards.section', $section) }}"
                           class="inline-flex items-center justify-center gap-1.5 rounded-lg border border-gray-300 dark:border-gray-600 px-3.5 py-2 text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors"
                           title="Download printable QR ID cards">
                        ...
                        <a href="{{ route('attendance.sheet', ['section' => $section, 'date' => $date->toDateString()]) }}"
                           class="inline-flex items-center justify-center gap-1.5 rounded-lg bg-gradient-to-r from-indigo-600 to-indigo-700 px-4 py-2 text-sm font-bold text-white hover:shadow-lg hover:shadow-indigo-500/30 transition-all">
                            Open Sheet
