<x-app-shell title="My Subjects">
    <div x-data="resourceModal({
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <p class="text-sm text-gray-600 dark:text-gray-400">Subjects you teach at your school
                @if ($activeSchoolYear ?? null) for <span class="font-semibold">SY {{ $activeSchoolYear->name }}</span>@endif.</p>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-white/5">
                        @forelse ($subjects as $subject)
                            <tr class="hover:bg-gray-50 dark:hover:bg-navy-700/30 transition-colors">
                                <td class="px-6 py-4"><code class="rounded-lg bg-gray-100 dark:bg-navy-900 px-2.5 py-1.5 font-mono text-xs text-gray-700 dark:text-gray-300">{{ $subject->code }}</code></td>
                                <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">{{ $subject->name }}</td>
                                <td class="px-6 py-4 text-gray-600 dark:text-gray-400">{{ $subject->gradeLevel?->name ?? '—' }}</td>
                                <td class="px-6 py-4 text-center tabular-nums text-gray-600 dark:text-gray-400">{{ $subject->units ?? '—' }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-1">
                                        <button type="button" @click='openEdit(@json($subject))' title="Edit subject"
                                        ...
                                        <x-confirm-delete :action="route('teacher.subjects.destroy', $subject)" title="Delete subject?" message="Delete {{ $subject->name }}? This cannot be undone." />
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="6" class="px-6 py-12 text-center">
                                ...
                                <p class="mt-1 text-xs text-gray-500">Add the subjects you teach this school year.</p>
                            </td></tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        {{-- Create / Edit modal --}}
        <x-form-modal create-title="Add Subject" edit-title="Edit Subject"
                      create-subtitle="Added to your school's subject list"
                      edit-subtitle="Update this subject"
                      submit-create="Add Subject">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="grade_level_id" x-model="form.grade_level_id" class="w-full rounded-lg border border-gray-300 dark:border-white/15 dark:bg-navy-900 text-sm px-4 py-2.5 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all">
                ...
                <div>
                    <label class="mb-1.5 block text-xs font-bold uppercase tracking-wider text-gray-600 dark:text-gray-400">Units</label>
                    <input type="number" name="units" x-model="form.units" min="0" max="20" placeholder="Optional"
                           class="w-full rounded-lg border border-gray-300 dark:border-white/15 dark:bg-navy-900 text-sm px-4 py-2.5 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all">
                    @error('units')<p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                </div>
            </div>
